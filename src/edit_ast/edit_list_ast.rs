...
pub fn get_elem_in_list(
    ...
    if let Some(list_pos) = val_list.get_pos_definition_value() {
        Ok(file_content.get(
            <TextRange as Into<Range<usize>>>
                ::into(list_pos))
        .ok_or(String::from("Impossible to read list"))?
        .strip_prefix('[').ok_or(String::from("Option is not a valid list"))?
        .strip_suffix(']').ok_or(String::from("Option is not a valid list"))?
