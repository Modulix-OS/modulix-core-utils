...
fn prefilter(query_lc: &str, query_len: usize, row: &RowView<'_>) -> bool {
    ...
    let attr_len = row.attr_lc.chars().count();
    attr_len.abs_diff(query_len) <= MAX_NAME_LEVENSHTEIN
}

#[cfg(feature = "app-info-gui")]
fn keywords_for(attr: &str) -> Vec<&'static str> {
    crate::package_info::package_basic_info::get_keywords(attr)
        .map(<[&str]>::to_vec)
        .unwrap_or_default()
}

#[cfg(not(feature = "app-info-gui"))]
fn keywords_for(_attr: &str) -> Vec<&'static str> {
    Vec::new()
}

/// Scans the whole index for `query`, scoring only the candidates the cheap
/// substring/length prefilter lets through (exact w.r.t. `score()`: nothing
/// it skips could have scored above 0 anyway). Returns `(score, row_index)`
